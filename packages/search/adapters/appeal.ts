// Апелляционные суды (*oblsud--*.sudrf.ru) — G2_ префикс
...
export class AppealSearchAdapter implements SearchAdapter {
  buildSearchUrl(req: SearchRequest): string {
    return buildSearchUrl(req, SEARCH_PARAMS.appeal);
  }

  async searchByCaseNumber(req: SearchRequest): Promise<SearchResult[]> {
    return parseResults(await smartFetch(this.buildSearchUrl(req), true), req);
  }
