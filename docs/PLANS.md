...
| INFR-001 | **ENOTFOUND судовых субдоменов (live).** Диагноз закрыт (2026-08-05): субдомены живы, DNS резолвится (84.42.111.139). Причины: (1) утренний ENOTFOUND — временный DNS-сбой; (2) WAF ГАС банит IP по rate-limit при интенсивных запросах (телефон/VPN с другим IP открывает msudrf без проблем; наш IP 196.61.180.141 после серии тестов получает 403 на sudrf+msudrf, бан временный); (3) `35.perm.msudrf.ru` до бана был доступен, но RuCaptcha-ключ невалиден (`ERROR_KEY_DOES_NOT_EXIST` — проверить ротацию); (4) sudrf-суды отвечают медленно (35-46с) — WAF-замедление. **Не баг кода.** Действия: IP без бана/whitelist, проверить RuCaptcha-ключ, в fetch-слое добавить backoff при 403 |
