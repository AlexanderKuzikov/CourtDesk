...
| Дата | Изменение |
|------|-----------|
| 2026-08-05 | **Диагностика live-ошибок + фикс runSingle.** Субдомены ГАС живы (DNS ок); утренний ENOTFOUND — временный DNS-сбой. district/appeal перепарсиваются (35-46с, WAF замедляет); appeal 33-8030 вышел из error→monitoring. msudrf нестабилен (403/reset, капча-пайплайн «Execution context destroyed»/«не найдены tab-content»). **NUM-002:** `runSingle` не фиксировал ошибки в деле (lastError оставался устаревшим) — добавлен catch с записью errorCount/lastError/status, ошибка пробрасывается в роут (500 PARSE_ERROR, не 409). Тесты 181, tsc/biome чисто. |
